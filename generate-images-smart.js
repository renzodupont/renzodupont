...
async function generateImageWithGemini(prompt, outputPath) {
  try {
    ...
    const fullPrompt = `CRITICAL REQUIREMENT: ABSOLUTELY NO TEXT, NO WORDS, NO LETTERS, NO TITLES, NO PHRASES, NO WRITING, NO CAPTIONS, NO LABELS anywhere in this image. Professional, high-quality featured image. Style: Modern, editorial, impactful, photorealistic. ZERO TEXT ALLOWED - purely visual imagery only. ${prompt}`;

    // Add negative prompting to explicitly exclude text elements
    const negativeElements =
      "text, words, letters, titles, captions, labels, writing, typography, fonts, headlines, subtitles, banners, signs, newspapers, books, documents, papers, screens with text";

    const response = await ai.models.generateImages({
      model: "imagen-4.0-generate-001",
      prompt:
        fullPrompt +
        ` Negative prompt: ${negativeElements}. Do NOT include: ${negativeElements}.`,
      config: {
        numberOfImages: 1,
      },
    });
...
function generateImagePrompt(metadata) {
  ...
  prompt +=
    "Professional, editorial style, high contrast, engaging composition. ABSOLUTELY FORBIDDEN: Any text, words, letters, titles, captions, labels, or written content. MUST BE 100% text-free visual imagery only. NO EXCEPTIONS.";
